  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-navy-900 text-white">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_88%_8%,rgba(245,104,22,0.16),transparent_28%),radial-gradient(circle_at_10%_92%,rgba(207,24,17,0.12),transparent_30%),linear-gradient(135deg,#0a101a_0%,#111827_55%,#20120f_100%)]" />
        <img
          src={logo}
          alt=""
          className="absolute -right-36 top-28 w-[min(62vw,42rem)] opacity-[0.08] mix-blend-screen"
        />
      </div>
        ...
        <div className="max-w-7xl mx-auto px-4 lg:px-8">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* SVR Phoenix Logo */}
              <div className="w-11 h-11 rounded-xl bg-black/20 border border-gold-400/20 flex items-center justify-center overflow-hidden shadow-lg shadow-gold-500/20">
            ...
            <nav className="hidden md:flex items-center gap-1">
              ...
              {NAV_ITEMS.map((item) => (
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${page === item.id
            ...
            >
              ...
              >
                ...
                {/* Dropdown Arrow */}
                <ChevronDown
                  className={`w-4 h-4 text-navy-300 transition-transform ${menuOpen ? 'rotate-180' : ''
                    }`}
                />
          ...
          <nav className="md:hidden flex items-center gap-1 overflow-x-auto pb-3 -mx-1">
            ...
            {NAV_ITEMS.map((item) => (
              ...
              <button
                key={item.id}
                onClick={() => setPage(item.id)}
                className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium whitespace-nowrap transition-all ${page === item.id
                    ? 'bg-gold-400/15 text-gold-300 border border-gold-400/20'
                    : 'text-navy-200 hover:text-white hover:bg-navy-700/50'
                  }`}
              >
