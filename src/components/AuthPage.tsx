...
export default function AuthPage({ onLogin }: AuthPageProps) {
  ...
  function handleLogin(e: React.FormEvent) {
    ...
    if (!account) {
      setError('No owner account found. Please create an owner account first.');
      return;
    }
    ...
    if (!idMatch || loginPass !== account.password) {
      setError('Invalid owner credentials. Please check your email/mobile and password.');
      return;
    }
  ...
  function handleRegister(e: React.FormEvent) {
    ...
    if (!reg.name.trim()) return setError('Owner name is required.');
    if (!reg.mobile.trim()) return setError('Mobile number is required.');
    if (!validateMobile(reg.mobile)) return setError('Please enter a valid 10-digit Indian mobile number.');
    if (!reg.email.trim()) return setError('Email address is required.');
    if (!validateEmail(reg.email)) return setError('Please enter a valid email address.');
    if (!reg.pgName.trim()) return setError('PG name is required.');
    if (!reg.pgAddress.trim()) return setError('PG address is required.');
    if (!reg.password) return setError('Password is required.');
    if (!validatePassword(reg.password)) return setError('Password must be at least 6 characters long.');
    ...
    <div className="min-h-screen bg-navy-900 text-white flex flex-col lg:flex-row">
      ...
      <div className="relative lg:w-1/2 min-h-[300px] lg:min-h-screen overflow-hidden bg-navy-800 flex flex-col justify-between p-8 lg:p-16">
        ...
        <div
          ...
          style={{
            background:
              'radial-gradient(ellipse at 70% 30%, rgba(200,164,92,0.12) 0%, transparent 50%), radial-gradient(ellipse at 30% 80%, rgba(30,77,139,0.2) 0%, transparent 50%), linear-gradient(135deg, #0a1628 0%, #0f2944 100%)',
          }}
        />
        ...
        <div className="relative z-10 animate-slide-up">
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="font-display font-bold text-xl tracking-tight">SVR</span>
          </div>
        </div>
        ...
        <div className="relative z-10 animate-slide-up delay-200">
          <h1 className="font-display font-bold text-4xl lg:text-5xl leading-tight mb-4">
            SVR <span className="gradient-gold">Co-Living</span>
            <br />
            PG Management
          </h1>
          <p className="text-navy-200 text-lg max-w-md leading-relaxed">
            Manage your entire PG from one place. Track rooms, beds, residents, and payments — all in one elegant dashboard built for PG owners.
          </p>

          <div className="mt-8 flex flex-wrap gap-4">
            {[
              { icon: Home, label: '48 Rooms' },
              { icon: Building2, label: '6 Floors' },
        ...
        <div className="relative z-10 text-navy-300 text-sm animate-fade-in delay-400">
          <p>PG Owner Management System</p>
        </div>
      </div>
...
function BuildingIllustrration() {
  return (
    <div className="absolute right-0 bottom-0 opacity-30 pointer-events-none">
      <svg width="400" height="500" viewBox="0 0 400 500" fill="none" className="animate-float">
        {/* Main building */}
        <rect x="80" y="80" width="240" height="380" rx="4" fill="#0f2944" stroke="#c8a45c" strokeWidth="1.5" opacity="0.6" />
        {/* Side building */}
        <rect x="320" y="180" width="60" height="280" rx="3" fill="#0a1628" stroke="#c8a45c" strokeWidth="1" opacity="0.4" />
        ...
        {/* Ground */}
        <rect x="0" y="460" width="400" height="40" fill="#0a1628" />
        {/* Door */}
        <rect x="180" y="420" width="40" height="40" rx="2" fill="#c8a45c" opacity="0.3" />
      </svg>
    </div>
  );
}

function SuccessView({ message, onContinue }: { message: string; onContinue: () => void }) {
  return (
    <div className="text-center animate-scale-in">
      <div className="w-20 h-20 rounded-full bg-green-500/20 flex items-center justify-center mx-auto mb-6">
        <CheckCircle2 className="w-10 h-10 text-green-400" />
      </div>
      <h2 className="font-display font-bold text-2xl text-white mb-3">Success!</h2>
      <p className="text-navy-200 mb-8">{message}</p>
      <button
        onClick={onContinue}
  ...
  return (
    <div className="animate-slide-up">
      <div className="mb-8">
        ...
        <h2 className="font-display font-bold text-3xl text-white mb-2">Owner Login</h2>
        <p className="text-navy-300 text-sm">Sign in to manage your PG property</p>
      </div>

      <form onSubmit={props.onSubmit} className="space-y-5">
        <div>
          <label className="block text-sm font-medium text-navy-100 mb-2">Email or Mobile Number</label>
        ...
        <div>
          <label className="block text-sm font-medium text-navy-100 mb-2">Password</label>
          <div className="relative">
            <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-navy-300" />
            <input
              type={props.showPassword ? 'text' : 'password'}
              value={props.loginPass}
              onChange={(e) => props.setLoginPass(e.target.value)}
        ...
        {forgotMsg && (
          <div className="flex items-start gap-2 text-sm text-navy-200 bg-navy-700/40 rounded-lg p-3 animate-fade-in">
            ...
            <span>Password recovery is not connected in this demo. Please use the demo owner account or reset the demo data.</span>
          </div>
        )}
...
function RegisterForm(props: {
  ...
  setReg: React.Dispatch<React.SetStateAction<{
    name: string;
    mobile: string;
    email: string;
    pgName: string;
  ...
  const update = (field: keyof typeof reg, value: string) =>
  ...
  return (
    <div className="animate-slide-in-right">
      <div className="mb-6">
        ...
        <h2 className="font-display font-bold text-3xl text-white mb-2">Create Owner Account</h2>
        <p className="text-navy-300 text-sm">Register as the PG owner to get started</p>
      </div>

      <div className="mb-5">
        <h3 className="text-xs font-bold text-gold-400 uppercase tracking-wider mb-3">PG Owner Details</h3>
      </div>

      <form onSubmit={props.onSubmit} className="space-y-4">
        <Field icon={User} label="Owner Name" value={reg.name} onChange={(v) => update('name', v)} placeholder="John Doe" required />
        <Field icon={Phone} label="Mobile Number" value={reg.mobile} onChange={(v) => update('mobile', v)} placeholder="9876543210" required />
        <Field icon={Mail} label="Email Address" value={reg.email} onChange={(v) => update('email', v)} placeholder="you@example.com" required />
        <Field icon={Home} label="PG Name" value={reg.pgName} onChange={(v) => update('pgName', v)} placeholder="SVR Co-Living PG" required />
        <Field icon={MapPin} label="PG Address" value={reg.pgAddress} onChange={(v) => update('pgAddress', v)} placeholder="123 MG Road, Bengaluru" required />
        ...
        <div>
          <label className="block text-sm font-medium text-navy-100 mb-2">Confirm Password</label>
          <div className="relative">
            <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-navy-300" />
            <input
